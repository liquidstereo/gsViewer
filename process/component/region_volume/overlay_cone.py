...
def _base_ring_pairs(
    n: int, half: np.ndarray, rotation: np.ndarray, center: np.ndarray,
...
def _side_pairs(
    ...
    locals_ = np.array([
        [+hx, 0.0, -hz], [0.0, 0.0, +hz],
        [-hx, 0.0, -hz], [0.0, 0.0, +hz],
        [0.0, +hy, -hz], [0.0, 0.0, +hz],
        [0.0, -hy, -hz], [0.0, 0.0, +hz],
    ], dtype=np.float32)
...
def paint_cone_region(
    ...
    paint_shape_region(
        ...
        compute_cone_segments(win, region),
        palette, selected,
    )

def make_cone_painter(
    plugin: Any, window: Any,
    palette: RegionPalette | None = None,
) -> Callable[[QPainter, int, int, np.ndarray | None], None]:
    return make_shape_painter(
        plugin, window, paint_cone_region, palette,
    )
